...
const addNewPromo = (body, picture) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(sqlQuery, [
name, price, id_name, description, id_size, id_delivery_method, disc, start_date, end_date, coupon_code, picture
])
        .then(({rows}) => {
            const response ={
                message: "Promo added",
...
const updatePromo = (query, body, picture) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(sqlQuery, [
name, price, description, id_size, id_delivery_method, disc, start_date, end_date, coupon_code, picture, id_name, id
])
        .then(({rows}) => {
            if(rows.length === 0){
                return reject({
